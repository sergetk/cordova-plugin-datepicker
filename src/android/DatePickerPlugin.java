 ...
 * @author Serge Tk (https://github.com/sergetk)
 * Implemented work around the Android's sdk 24 bug where spinner themes are ignored only for date picker not time picker.
 */
...
public class DatePickerPlugin extends CordovaPlugin {
         ...
         public void run() {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
               ...
               if(Build.VERSION.SDK_INT == Build.VERSION_CODES.N) {
                  ...
                  dateDialog =  new FixedHoloDatePickerDialog(new ContextThemeWrapper(currentCtx, android.R.style.Theme_Holo_Light_Dialog), dateSetListener, jsonDate.year, jsonDate.month, jsonDate.day);

                  prepareDialog(dateDialog, dateSetListener, callbackContext, currentCtx, jsonDate);
               } else {

                  dateDialog = new DatePickerDialog(currentCtx, theme, dateSetListener, jsonDate.year, jsonDate.month, jsonDate.day);
                  prepareDialog(dateDialog, dateSetListener, callbackContext, currentCtx, jsonDate);
               }
   ...
   private static final class FixedHoloDatePickerDialog extends DatePickerDialog {
      private FixedHoloDatePickerDialog(Context context, OnDateSetListener callBack, int year, int monthOfYear, int dayOfMonth) {
